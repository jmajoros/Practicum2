...
P2$y<-factor(P2$y)
summary(P2)
P2$job<-sub("^$", "unknown", P2$job)


#prop.test to see if marital status is sig diff. in two pops.
prop.test(x=c(421*0.14,9064), n=c(421, 45211),
          conf.level=0.95)
#Very different
...
```
Discuss the potential means here.
...
abline(0,1,lty=2)
```
Not great at predicting...Specificity is good, sensitivity is terrible. So we know some things that make people more likely to buy, but don't have a great model.  Perhaps contingency tables will be better....we could still see how it performs on the test set or just use the full set above.
...
##Notes:  1) segment age and balance to make into ordinal factors
          ...
          young, tertiary education, etc - ie., perfect buyer - and see prob they will buy.
