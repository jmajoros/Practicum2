...
The purpose of this report is to use confidence intervals, logistic regression, as well as analysis of contingency tables in order to find the likelihood that the USB Toaster will be a successful product, useful demographic points to aid in formulating a successful marketing campaign, and potential profit or loss. By calculating confidence intervals, we found that 95% of the time, the true probability of the USB Toaster being profitable falls between 0.118 and 0.124. In our logistic regression model, our final model included a binary response variable for product response (yes or no) and explanatory variables education, job, age, balance, housing, loan, default, contact, and marital status. We fit this model on a training set of data taken from the dataset provided by the consulting firm. We found the correct classification rate for the training data set to be 88.2%. Using contingency tables and the logistic regression model, we found the estimated profit of the USB Toaster to be between 708,470z and 1,570,662z where z is fixed price - fixed cost.

# Introduction

Launching a new product is an expensive undertaking that a company enters into, understanding that risk is inherently involved. However, equally understood is the need to mitigate the risk of incurring a loss. The difference between a bottom line that is in the red or in the black depends on the ability to make well-educated decisions based on quantifiable data, and an assessment of the potential risks and rewards for launching a given product. All of this hinges on having quality information that has been delivered in a manner that makes it intelligible.  

Here, we have rendered a detailed analysis of the proposed new product, the USB Toaster, based on the comprehensive data gathered by our firm. We have compiled an examination of the data collected in both samples and highlighted which demographic data points may be of interest, and most importantly, we predicted how successful the USB Toaster will be based on our comprehensive model. The processes of reaching these objectives will be detailed in sections covering the data, exploratory data analysis, and the model further on in this report.
...
The data we have analyzed comes from two different sources, the client and the consulting firm. The data obtained by the client provides valuable information on demographics that may or may not have interest in purchasing the USB Toaster, although it is clearly not representative of the population. We found every variable in this dataset to be a large departure from the general population through additional research and contrasting it with the sample collected through the DOE designed experiment, which is though to represent the target market. In light of these discoveries, we focused our analysis on the data obtained by the consulting firm, which would provide more insight into the USB Toaster product market. The data collected through a random sample by the consulting firm proved to be representative of the test market population for the USB Toaster, seeing as this data sample was 100 times larger that that collected by the client. We therefore used this dataset to achieve our specific aims.

## Examining Missing Data

The original data obtained by the consulting firm included 45,211 observations of 11 potential variables of interest. There were a somewhat considerable amount of missing values in this dataset, therefore we tested these proportionally against the population to see if there were any significant differences. We found that there was no evidence that MOST missing values come from different distributions; we did find that missing values (labeled "unknown") for education level was statistically different from the population, although not meaningfully so. We also found that unknown contact method was strongly significantly different. We subsequently retained the missing/unknown values in this dataset in our analysis.
...
*There is evidence that "unknown" as an education level is statistically different, but not meaningfully so. There is very strong evidence that contact method unknown is different.*
...
```{r, echo=FALSE}
write.csv(P2.final,"P2.csv", row.names = FALSE)
```
...
#P2<-read.csv("~/Documents/IU-2020-2021/Spring-2021/Consulting/Practicum2")
...
#dat2 <- read.csv("~/Documents/IU-2020-2021/Spring-2021/Consulting/Practicum2", header=T, na.strings=c(c("unknown", ""),"NA"))
dat2 <- read.csv("P2.csv", header=T, na.strings=c(c("unknown", ""),"NA"))
...
```
There is essentially a 0% chance with the target market.  Through further analysis, better targeting, and price raising might lead to a profit, there is a chance this will increase. 
...
sumary(m1.final)
```
Our model is not a good fit via the Hosmer-Lemeshow statistic for Goodness-of-fit used for binary logistic regressions. Figure X demonstrates the model's fit, where the values of the predicted probabilities align with the observed values. The farther from the line the points are, the worse the fit. The data points have 95% confidence intervals reflected in the lines, where most points at least fall in the 95% interval. 
...
```{r, echo=FALSE}
sumary(m3.3)  #we see student/retired, tertiary, divorced/single, no house
...
- I think its ok to stick with the previous model and contigency tables above
```{r, echo=FALSE}
...
- I think this is enough, we can discuss it as a client's method of considering price
...
```{r}
dat3 <- read.csv("~/Documents/IU-2020-2021/Spring-2021/Consulting/Practicum2", header=T, na.strings=c(c("unknown", ""),"NA"))
